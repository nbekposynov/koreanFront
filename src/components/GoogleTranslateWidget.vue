...
export default {
  data() {
    return {
      showDropdown: false, // Контролирует видимость выпадающего списка
      selectedLanguage: '', // Хранит выбранный язык
    };
  },
  methods: {
    toggleLanguageDropdown() {
      this.showDropdown = !this.showDropdown; // Переключает видимость выпадающего списка
    },
    setLanguage(language) {
      this.selectedLanguage = language; // Обновляет выбранный язык
      this.triggerTranslate(); // Вызывает метод перевода
      this.showDropdown = false; // Скрывает список после выбора
    },
    triggerTranslate() {
      // Эмулирует изменение в виджете Google Translate
      const translateWidget = document.querySelector(".goog-te-combo");
      if (translateWidget) {
        translateWidget.value = this.selectedLanguage; // Устанавливает выбранный язык
        translateWidget.dispatchEvent(new Event('change')); // Инициирует событие 'change'
      }
    }
  }
}
...
.dropdown div {
  color: black; /* Черный цвет текста для лучшей видимости */
  padding: 10px 0;
...
.dropdown div:hover {
  background-color: #f1f1f1; /* Эффект наведения для элементов списка */
}
</style>