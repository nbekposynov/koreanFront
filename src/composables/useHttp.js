...
export const useHttp = () => {
  ...
  async function getNewsPaginate(params) {
    try {
      const response = await http.get("api/postPaginate", { params });
      return response.data; // Предполагается, что API возвращает данные напрямую
    } catch (error) {
      console.error("Ошибка при получении списка новостей:", error);
      throw error; // или возвращаем пустой список/объект в зависимости от вашей логики обработки ошибок
    }
  }
  ...
  async function getMagazines(year, month) {
    const res = await http.get(
      month
        ? `api/magazines/year/${year}/month/${month}`
        : `api/magazines/year/${year}`
    );
    return res.data;
  }

  async function downloadMagazine(id) {
    const res = await http.get(`api/book/download/${id}`, {
      responseType: "blob",
    });
    console.log(res);
    return res.data;
